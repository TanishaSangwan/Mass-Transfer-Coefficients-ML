...
y = df['k_L (m/s)'].values.reshape(-1,1)   # IMPORTANT reshaping for scaling
...
mlp = MLPRegressor(
    hidden_layer_sizes=(64,),   # stronger model
    activation='relu',
    solver='adam',
...
plt.scatter(y_test_real, y_pred, color='purple', alpha=0.7, edgecolors='k')
plt.plot([y_test_real.min(), y_test_real.max()],
         [y_test_real.min(), y_test_real.max()], 'r--', linewidth=2)
plt.xlabel("Actual kL (m/s)")
plt.ylabel("Predicted kL (m/s)")
plt.title("MLP Neural Network: Actual vs Predicted kL (No Log, Scaled Target)")
